...
type t = {
  raw: string option;  (* needed for KEXDH_REPLY signature hash *)
  protoversion: string;
...
let unmarshal ofd = 
  ...
  if String.sub str 0 4 <> "SSH-" then raise Parse_failure;
  let str' = String.sub str 4 (String.length str - 4) in
  let other_dash = try String.index str' '-' with Not_found -> raise Parse_failure in
